...
class MatchList extends React.Component {
    ...
    //主列表
    buildGameList = (list,detailList) => {
        let gameList = list.map((item) => (
            <li key={item.GameID} className={style.matchItem} onClick={()=>this.showGameDetail(item.GameID)}>
                ...
                <div>{detailList?this.buildDetailList(detailList):""}</div>
    ...
    showGameDetail = (gameId) => {
        this.props.showGameDetail(gameId)
    }
    ...
    render() {
        return (
            <ul className={style.gameList}>{this.buildGameList(this.props.gameInfos,this.props.gameItems)}</ul>
        );
    }
}
