    ...
    tabContent = (attributeId, list) => {
        ...
        if (list) {
            return (<div className="list-item">
                <span onClick={() => self.props.chooseAttribute(this.props.selectEnter,this.props.paramSectionType, attributeId, "")}>不限</span>
                {list.map((item, index) => (
                    <span key={index} onClick={() => self.props.chooseAttribute(this.props.selectEnter,this.props.paramSectionType, attributeId, item)}>{item}</span>
                ))}
            </div>)
        } else {
            return ""
